...
| 支付 | 统一支付工厂 + Stripe/Payermax | 简化为虎皮椒 xunhupay 单一通道 |
...
响应（200）：
{
  ...
  "url": "https://api.xunhupay.com/...",   // 手机端跳转支付页
  "url_qrcode": "https://...",             // PC 端二维码图片地址（可选）
  "expire_at": "2026-03-31T12:30:00"
}
```

#### POST `/api/v1/payment/notify` — 虎皮椒异步回调

```
请求（由虎皮椒服务器 POST，form 表单）：
  trade_order_id, total_fee, transaction_id, open_order_id,
  order_title, status（OD=已支付）, appid, time, nonce_str, hash 等

处理逻辑：
  1. 按虎皮椒规则验证 hash（参数字典序拼接 + APPSECRET 后 MD5）
  2. status=OD 时校验 total_fee 与本地订单金额一致
  3. 更新订单状态为 paid，记录 provider_order_id
  4. 响应纯文本 success
...
### 6.1 简介
虎皮椒（xunhupay.com）面向个人/站长的微信、支付宝收款接口；微信与支付宝在后台各申请一条支付渠道，各得一对 `APPID` + `APPSECRET`。

- 官网：https://www.xunhupay.com/
- 商户登录：https://admin.xunhupay.com/
- 注册账号：https://admin.xunhupay.com/sign-up.html
- 忘记密码：https://admin.xunhupay.com/forgot-password.html
...
用户点击"购买报告"
    │
    ▼
前端 POST /api/v1/payment/create（pay_method: wechat | alipay）
    │
    ▼
后端创建本地订单（MySQL，状态=pending）
    │
    ▼
后端 POST 虎皮椒 payment/do.html（form + hash 签名）
    │
    ▼
返回 url / url_qrcode 给前端
    │
    ▼
手机跳 url；PC 可展示 url_qrcode 图片
    │
    ▼
用户完成支付
    │
    ├── 虎皮椒异步回调 POST /api/v1/payment/notify
    │       │
    │       ▼
    │   后端验 hash → 更新订单为 paid
    │
...
### 6.3 虎皮椒 API 要点（详见官方文档）

创建支付：`POST https://api.xunhupay.com/payment/do.html`（`application/x-www-form-urlencoded`）

必填字段包括：`version`、`appid`、`trade_order_id`、`total_fee`、`title`、`time`、`notify_url`、`nonce_str`、`hash`。

`hash`：非空参数按 key 字典序拼成 `k=v&...`，末尾拼接 `APPSECRET`，再 MD5（32 位小写）；`hash` 自身不参与签名。

回调：`status=OD` 表示已支付；同样用 `hash` 验签。

### 6.4 注意事项
- 开户费与平台通道费以官网说明为准；另含微信/支付宝官方费率
- 回调 URL 须 HTTPS 公网可访问（本地可用 ngrok）
- 环境变量：`XUNHUPAY_APPID_WECHAT`、`XUNHUPAY_APPSECRET_WECHAT`、`XUNHUPAY_APPID_ALIPAY`、`XUNHUPAY_APPSECRET_ALIPAY`、`XUNHUPAY_NOTIFY_URL`
...
CREATE TABLE orders (
    ...
    provider_order_id VARCHAR(64) DEFAULT NULL COMMENT '支付渠道侧订单号',
